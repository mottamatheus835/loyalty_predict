...
tb_agg_transacao AS  (
    SELECT IdCliente,
        ...
        SUM(DISTINCT CASE WHEN dtDia >= date('2025-10-01', '-7 day')  AND qtdePontos < 0 THEN  qtdePontos ELSE 0 END) AS   qtdePontoNegD7,
        ...
        SUM(DISTINCT CASE WHEN dtDia >= date('2025-10-01', '-56 day') AND qtdePontos < 0  THEN qtdePontos ELSE 0  END) AS  qtdePontoNegD56,
...
tb_hora_cliente AS (    
    SELECT 
        ...
        SUM(CASE WHEN dtDia >= DATE('2025-10-01', '-7 day') THEN duracao END) AS qtdeHorasD7,
        SUM(CASE WHEN dtDia >= DATE('2025-10-01', '-14 day') THEN duracao END) AS qtdeHorasD14,
        SUM(CASE WHEN dtDia >= DATE('2025-10-01', '-28 day') THEN duracao END) AS qtdeHorasD28,
        SUM(CASE WHEN dtDia >= DATE('2025-10-01', '-56 day') THEN duracao END) AS qtdeHorasD56
    FROM tb_horas_dia
